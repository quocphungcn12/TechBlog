...
namespace TechBlog.Data.Services
{
    public class RoyaltyService(UserManager<AppUser> userManager, IUnitOfWork unitOfWork, IConfiguration configuration) : IRoyaltyService
    {
        #region Fields
        ...
        {
            using var conn = new SqlConnection(_configuration.GetConnectionString(SystemConstants.ConnectionString.TechBlogDB));
        ...
        {
            var fromUser = await _userManager.FindByIdAsync(fromUserId.ToString()) ?? throw new ArgumentException($"User {fromUserId} not found");
            var toUser = await _userManager.FindByIdAsync(toUserId.ToString()) ?? throw new ArgumentException($"User {toUserId} not found");
